...
export const MapMemo = memo((props) => {
  ...
  const containerStyle = {
    width: '100%',
    height: `${
      window.innerWidth > 600 ? 'calc(100vh - 60px)' : 'calc(100vh - 360px)'
    }`,
    styles: [
      {
        featureType: 'all',
  ...
  return (
    <GoogleMap
      ...
      options={{
        ...
        disableDefaultUI: window.innerWidth > 600 ? false : true, // Hides the default Google Maps controls
      }}
      //onUnmount={onUnmount}
    >
      <></>
...
export const Map = memo((props) => {
  ...
  const onLoad = useCallback(function callback(map) {
    ...
    const bostonBounds = {
      north: 42.396, // Latitude of the northern boundary
      south: 42.227, // Latitude of the southern boundary
      east: -70.825, // Longitude of the eastern boundary
      west: -71.191, // Longitude of the western boundary
    }
    map.fitBounds(bostonBounds)
    createPins(data, map, bostonBounds)
    // setMap(map)
  }, [])
  const createPins = useCallback((data, map, bounds) => {
    data.map((clam, i) => {
      ...
      window.google.maps.event.addListener(pin, 'click', () => {
        // scrollTo(pin.get('id'))
        dispatch(setHoverId(pin.get('id')))
        hoverHandlerCallback(pin.get('id'))
      })
